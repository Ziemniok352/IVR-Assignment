...
class image_converter:
    ...
    def __init__(self):
        ...
        self.end_effector_pos = rospy.Publisher(
            "end_effector_pos", Float64MultiArray, queue_size=10)
        ...
        ts = message_filters.TimeSynchronizer(
            [self.image_sub1, self.image_sub2], 5)
        ts.registerCallback(self.callback)

    def publish_angles(self, angles):
        # Initializes publishers
        robot_joint2_est = rospy.Publisher(
            "/image1/joint2_position_estimator", Float64, queue_size=10)
        robot_joint3_est = rospy.Publisher(
    ...
    def get_3d_coords(self, image1, image2):
        ...
        if self.image1_coords[2][0] == 0:
            self.green = np.array(
                [self.image2_coords[2][0], 0, self.image2_coords[2][1]])
        if self.image2_coords[2][0] == 0:
            self.green = np.array(
                [0, self.image1_coords[2][0], self.image1_coords[2][1]])
        if self.image1_coords[2][0] != 0 and self.image2_coords[2][0] != 0:
            self.green = np.array([self.image2_coords[2][0], self.image1_coords[1][0], np.mean(
                [self.image1_coords[2][1], self.image2_coords[2][1]])])

        # Calculate red
        if self.image1_coords[3][0] == 0:
            self.red = np.array(
                [self.image2_coords[3][0], self.green[1], self.image2_coords[3][1]])
        if self.image2_coords[3][0] == 0:
            self.red = np.array(
                [self.green[0], self.image1_coords[3][0], self.image1_coords[3][1]])
        if self.image1_coords[3][0] != 0 and self.image2_coords[3][0] != 0:
            self.red = np.array([self.image2_coords[3][0], self.image1_coords[3][0], np.mean(
                [self.image1_coords[3][1], self.image2_coords[3][1]])])
    ...
    def get_angles(self, image1, image2):
        ...
        self.angles = np.array([self.estimate_angle(self.coords[2], self.coords[1], 3.5, 'x'), self.estimate_angle(
            self.coords[2], self.coords[1], 0, 'y'), self.estimate_angle(self.coords[3], self.coords[2], 3, 'x')])
        return self.angles
    ...
    def x_fun(self, theta, a, b):
        m = np.array([[1, 0, 0], [0, np.cos(theta), -np.sin(theta)],
    ...
    def detect_end_effector_pos(self):
        ...
        robot_end_pos_pub = rospy.Publisher(
